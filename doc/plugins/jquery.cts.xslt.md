...
**eg** : We instantiate CTS.XSLT "llt.tokenizer" which has fields name : xml, splitting, merging. Instead of generating new DOM, we can give it the following driver parameters.
```javascript
driver : {
...
**eg** : We instantiate CTS.XSLT "llt.tokenizer" which has fields name (default value): splitting(true), merging(false). In our case, we want merging to be true by default:
```javascript
defaults : {
    merging : true
}
...
**eg** : We instantiate CTS.XSLT "llt.tokenizer" which has fields name : splitting, merging.
```javascript
defaults : {
    merging : "merging"
}
...
Callback is called when XSLT is called and has answered. Parameter passed to this function are value returned by the XSLT.
...
<script type="text/javascript">
  $(document).ready(function() {
    $("#lltTokenizer").ctsXSLT("llt.tokenizer", {
      "endpoint" : "http://XSLTs.perseids.org/llt/segtok",
      "driver" : {
        "text" : $(".TEItext")
      },
      "click" : ".button",
    ...
    $("#lltTokenizer").on("cts-XSLT:llt.tokenizer:done", function() {
      //console.log(data);
    });

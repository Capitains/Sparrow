  ...
  $.extend(SparrowTypeahead.prototype, {
    checkURI : function() {
      ...
      if("text_uri" in GET) {
        ...
        self.element.val(GET["text_uri"]);
        self.text = new CTS.text.Passage(GET["text_uri"], new CTS.endpoint.simpleUrl(GET["text_uri"]));
        //We load the text
        self.text.retrieve({
          success : function(data) {
            if(self.text.checkXML() === true) {
              //We feed our targer value
              $target.val(self.text.getXml(self.settings.retrieve_scope, "string"));
              //We reset legend of the button
              $target.trigger("cts-passage:retrieved", self.text);
            } else {
              $target.val(data);
              $target.trigger("cts-passage:retrieved", self.text);
            }
